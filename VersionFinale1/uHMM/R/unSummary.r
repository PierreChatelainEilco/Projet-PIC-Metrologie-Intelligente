...
#' @param repertory repertory in which the summary must be saved
#' @param summaryLabel title of the summary window
#' @author Paul
#' @import tcltk tcltk2

.unSummary<-function(data,repertory=NULL,summaryLabel="Summary"){
	...
	if(!is.null(repertory)){
		write.table(leSummary,file=paste(repertory,"summaryData.txt",sep=""),row.names=FALSE)
	}
}
...
if (any(is.na(fixDataTemp)) & any(is.na(impData))){# s'il y a des NA dans les 2 tableaux
  if(any(fixDataTemp!=impData,na.rm=TRUE) | any(which(is.na(fixDataTemp))!=which(is.na(impData))) ){
    impData<-fixDataTemp
    datefix<-format(Sys.time(), "%d_%m_%Y_%H_%M_%S")
...
if(as.character(dispTab)[1] == as.character(win1$env$prediction)[1]){assign("rawValidData",impData,envir=uHMMenv)}
