...
.modelingTab<-function(tm,leftMargin=30,
  ...
  runButton<-tk2button(win1$env$modelisation,text=tm$runLabel,image="run",compound = "left",command=function(){
    ...
    if(!exists("nbSymbols",where=uHMMenv)){
      ...
      #Assignation de chaque nouvelle donnee a son symbole le plus proche
      #Display in console
      ...
      .graphicsByState(data=uHMMenv$rawData[uHMMenv$trainingRows,],period=uHMMenv$trainingPeriod,stateSeq=uHMMenv$MstateSeq,step="modeling",directory=uHMMenv$saveDirectory,uHMMinterface=TRUE,tm=tm,graphicFrame=graphicFrame)

      
      # result saving
      ModelingSavingTime<-format(Sys.time(), "%d_%m_%Y_%Hh%Mmin")
      lastModelName<-paste("MarkovEstimationOutput_",ModelingSavingTime,sep="")
      
      save(MstateSeq,selectedNames,estimatedHMM,symbCentersNorm,symbCenters,gap,normParams,envir=uHMMenv,file=paste(uHMMenv$rfilesOutput,lastModelName,sep=""))
