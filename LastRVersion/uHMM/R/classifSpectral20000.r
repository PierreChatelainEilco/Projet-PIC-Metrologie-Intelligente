...
#' @seealso \code{\link{computeFastSpectralJordan}}
#' @export
...
.spectralClassifGT20000<-function(data,normalization=TRUE,varEx=0.95,stateNb=0,uHMMinterface=FALSE,tm=NULL,console=NULL){
  ...
  xf <- data[!toRemove,]
	...
	res=computeFastSpectralJordan(xf, nK, Kech=nrow(xf), StopCriteriaElbow=varEx, neighbours=7, uHMMinterface=uHMMinterface,console=console,tm=tm);
...
.spectralClassifLT20000<-function(data,normalization=TRUE,varEx=0.95,stateNb=0,uHMMinterface=FALSE,tm=NULL,console=NULL){
	...
	KElbow=KmeansAutoElbowDicotomie(features=xf,Kmax=nrow(xf),StopCriteria=varEx,graph=F)
	nbProto=KElbow$K;
