...
.graphicsByState<-function(data,stateSeq,step=NULL,period,directory,uHMMinterface=FALSE,hscaleGraphicFrame=1.2,vscaleGraphicFrame=1.2,tm,graphicFrame){
  ...
  if (dir.exists(paste(directory,tm$diagramsRepertoryLabel,.simpleCap(step),"/",sep=""))==0){
    dir.create(path=paste(directory,tm$diagramsRepertoryLabel,.simpleCap(step),"/",sep=""), recursive = TRUE)
    }
  #assign(paste(step,"DiagramsOutput",sep=""),paste(directory,tm$diagramsRepertoryLabel,.simpleCap(step),"/",sep=""),envir=.GlobalEnv)
  
 out<-paste(directory,tm$diagramsRepertoryLabel,.simpleCap(step),"/",sep="")
  ...
  for(a in 1:nbVar){    #Pour chaque parametre
    ...
    figure=paste(out,paste("boxplot",colnames(data2)[a],".jpg",sep=""),sep="")
    jpeg(figure,700,700)
    ...
    figure=paste(out,paste("Representation_",colnames(data2)[a],".jpg",sep=""),sep="")
    jpeg(figure,700,700)
  ...
  figure=paste(out,tm$seqClassifFileTitle,sep="")
  jpeg(figure,700,700)
  ...
  for(a in 1:nbVar){ #Pour chaque parametre
    ...
    figure=paste(out,paste("boxplot",colnames(data2)[a],"_Reduit.jpg",sep=""),sep="")
    jpeg(figure,700,700)
  ...
  #Sauvegarde
  write.table(summaryTable,file=paste(paste(directory,tm$diagramsRepertoryLabel,"/",sep=""),"summaryTable",.simpleCap(step),".xls",sep=""),row.names=FALSE)
