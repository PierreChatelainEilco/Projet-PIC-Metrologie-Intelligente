...
.representationPlot<-function(data,period,tm,varNames){
	...
	varList<-tk2listbox(selectionFrame, height=7, selectmode="single", background="white")
...
.savePlot<-function(data,period,tm,output){
  ...
  if (dir.exists(paths=paste(output,tm$selectedRawDataRepertory,sep=""))==0){
    dir.create(path=paste(output,tm$selectedRawDataRepertory,sep=""), recursive = TRUE)
  }
  
  for(i in 1:ncol(data)){
    
    figure<-paste(output,tm$selectedRawDataRepertory,colnames(data)[i],".jpg",sep="")
    jpeg(figure,750,600)
...
.PCAwindow<-function(data,tm){
  ...
  ACPdudi<-PCA(X=data,scale.unit = TRUE,ncp=ncol(data),graph=FALSE)
  
  monACP<-tkrplot(fenetrePlotACP,hscale=2.5,vscale=2,function()plot(ACPdudi, axes = c(1, 2), choix = "var",cex.axis=1.5,cex.lab=1.5,cex=1.5))
      ...
      if(Dim2Ecrit>0){
        if(Dim1Ecrit<(ncol(data)+1)){
          if(Dim2Ecrit<(ncol(data)+1)){
            ...
            monACP<-tkrplot(fenetrePlotACP,hscale=2.5,vscale=2,function()plot(ACPdudi, axes = c(Dim1Ecrit, Dim2Ecrit), choix = "var",cex.axis=1.5,cex.lab=1.5,cex=1.5))
            tkgrid(monACP,row=0,sticky="w")
            tkconfigure(monACP, bg="white")
